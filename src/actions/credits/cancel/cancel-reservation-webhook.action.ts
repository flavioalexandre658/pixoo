...
*/
      ...
      if (!reservation) {
        console.error(
          `❌ Reserva não encontrada para cancelamento via webhook:`,
          {
            reservationId,
            userId,
            timestamp: new Date().toISOString(),
      ...
      if (reservation.status === "confirmed") {
        console.error(
          `❌ Tentativa de cancelar reserva já confirmada via webhook:`,
          {
            reservationId,
            userId,
            timestamp: new Date().toISOString(),
      ...
      await db
        ...
        .set({
          status: "cancelled",
          description:
            reason || "Cancelado via webhook devido a falha na geração",
          updatedAt: new Date(),
        })
